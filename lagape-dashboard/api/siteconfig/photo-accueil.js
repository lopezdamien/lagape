...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'POST') {
      ...
      const data = (await kvGet(configKey)) || { photoAccueil: null }

      if (files.photo) {
        const file = Array.isArray(files.photo) ? files.photo[0] : files.photo
        const buffer = await fs.readFile(file.filepath)
        const name = file.originalFilename || 'photo-accueil.jpg'
        
        // Delete old blob if exists
        if (data.photoAccueil) {
          await deleteBlob(data.photoAccueil).catch(() => {})
        }
        ...
        data.photoAccueil = blob.url
        await fs.unlink(file.filepath).catch(() => {})
      }

      await kvSet(configKey, data)
      return res.json({ photoAccueil: data.photoAccueil })
    }
